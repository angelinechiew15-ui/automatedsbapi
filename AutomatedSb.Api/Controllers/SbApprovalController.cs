        ...
        [FromQuery] string? status)
    {
        ...
        const string sql = @"
            SELECT
                ...
                NVL(a.sb_status, 'NO_STATUS') AS approval_status
            FROM cm_matrix_sb s
            ...
            LEFT JOIN (
                SELECT z.sb_id, z.sb_status
                FROM (
                    SELECT
                        a.cm_matrix_sb_approval_sb_id AS sb_id,
                        a.cm_matrix_sb_approval_sb_status AS sb_status,
                        ROW_NUMBER() OVER (
            ...
            WHERE s.cm_matrix_sb_valid = 'Y'
              ...
              AND (:status IS NULL OR NVL(a.sb_status, 'NO_STATUS') = :status)
